    ...
    const handleGetAllInventory = async () => {
        ...
        try {
          ...
          if (response?.data?.status === true) {
            setGetAllInventory(response?.data?.data?.inventoriesResponse);
            setFilteredAllInventory(response?.data?.data?.inventoriesResponse); 
      ...
      useEffect(() => {
        setFilteredAllInventory(
            getAllInventory?.filter(emp => emp?.boxName?.toLowerCase().includes(searchQuery.toLowerCase()))
        );
                    ...
                    {screenWidth> 450 &&
                        ...
                        <div className='sm:w-[80%] w-[100%] pl-3 pr-3 sm:pl-10 overflow-y-scroll flex-col flex h-[75vh] pt-5 sm:pr-10'>
                          ...
                          >
                            ...
                            className='flex  justify-center items-center sm:w-[140px] w-[140px] h-[40px] sm:ml-5 ml-2 sm:rounded-full rounded-full bg-colorinput bg-[#3045A3] text-[white] sm:text-[15px] text-[12px]'
                          >
                            ...
                            Create Box
                                ...
                                <select 
                                    ...
                                    <option value="box">Sort by Box</option>
                                </select>
                                </div>
                            </div>
                                {(screenWidth <440 && role === "admin")  &&
                          ...
                          >
                            ...
                            {screenLoader ? (
                                ...
                                <>
                            <DataTableManageInventory filteredAllInventory={filteredAllInventory} sortOption={sortOption}  setGetAllInventory={setGetAllInventory} editMode={editMode} setEditMode={setEditMode} updateData={updateData} setUpdateData={setUpdateData} />
                            </>
)}
                        </div>
                    </div>
